...
<div class="container">
     ...
     <table class="table">
         <tbody>
            ...
            <c:if test = '${boardFile != "null"}'>
            	<tr>
	            	<td>board_file :</td>
	            	<td><a href="${pageContext.request.contextPath}/upload/${boardFile.boardFileSaveName}.${boardFile.boardFileExt}">
	            	${boardFile.boardFileSaveName}.${boardFile.boardFileExt}</a></td><!-- 만약 originName을 가져오면, 다운로드를 할 파일을 찾을 수 없다 -->
            	</tr>
            </c:if>
        </tbody>
    </table>
	...
	<c:forEach items="${boardCommentList}" var="bc">
		<div>
			...
			<div><a href="${pageContext.request.contextPath}/board/removeBoardComment?boardCommentNo=${bc.boardCommentNo}&boardNo=${board.boardNo}">삭제</a></div>
		</div>
	</c:forEach>
	<!-- 댓글 입력 폼-->
	<hr>
